...
bool Entity::checkPlatformCollision(sf::Sprite object)
{
	...
	if (object.getPosition().y < sprite->getPosition().y && object.getPosition().y + object.getLocalBounds().height > sprite->getPosition().y && object.getPosition().x < sprite->getPosition().x && object.getPosition().x + object.getLocalBounds().width + sprite->getLocalBounds().width * 2 > sprite->getPosition().x)
		return true;
}
