...
bool Hero::checkPlatformCollision(sf::Sprite object)
{
	...
	if (object.getPosition().y < sprite->getPosition().y && object.getPosition().y + object.getLocalBounds().height + 20 > sprite->getPosition().y && object.getPosition().x < sprite->getPosition().x && object.getPosition().x + object.getLocalBounds().width * Constants::PLATFORM_W_SCALE + sprite->getLocalBounds().width * 1 > sprite->getPosition().x)
	{
		return true;
	}
}
...
int Hero::checkPlatformVectorCollision(std::vector<Platform *> platforms)
{
	...
	{
		if (checkPlatformCollision(platforms[i]->sprite))
			return i;
	}
	return -1;
}
